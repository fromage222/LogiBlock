// main.js — LogiBlock client (v2 — with animations & confetti)
...
function showScreen(screenId) {
  ...
  // Re-trigger CSS animation on screen switch
...
joinRoomBtn.addEventListener('click', () => {
  ...
  if (!code || code.length !== 6 || !/^\d+$/.test(code)) {
    showJoinError('Please enter a valid 6-digit room code'); return;
  }
  clearJoinError();
  myPlayerName = name;
...
function renderLobbyUpdate(state) {
  ...
  if (amIHost) {
    ...
    if (state.selectedPuzzleName) {
      ...
      const displayName = diffLabel ? `${state.selectedPuzzleName} — ${diffLabel}` : state.selectedPuzzleName;
      ...
      cell.addEventListener('click', () => {
        ...
        if (selectedShapeId) {
          ...
          socket.emit('game:move', {
            action: 'place', shapeId: selectedShapeId,
            rotation: selectedRotation, originRow, originCol,
          });
...
function getCursorEl() {
  if (!_cursorEl) {
    ...
    _cursorEl.style.cssText = [
      ...
      'background:rgba(255,255,255,0.93)', 'border:2px solid #ff5533',
...
function renderLeaderboard(entries) {
  ...
  if (filtered.length === 0) {
    ...
    tbody.innerHTML = filtered.map(e =>
      `<tr><td>${e.rank}</td><td class="leaderboard-time">${e.time}</td><td>${e.playerNames.join(', ')}</td></tr>`
    ).join('');
  }
}
...
socket.on('game:error', (message) => { showGameError(message); });
...
socket.on('randomMode:event', ({ type, description } = {}) => {
  ...
  if (type === 'blind_bank') {
    ...
    blindInterval = setInterval(() => {
      remaining--;
      countdownEl.textContent = `Blind! ${remaining}s`;
      if (remaining <= 0) { clearInterval(blindInterval); blindInterval = null; }
    }, 1000);
